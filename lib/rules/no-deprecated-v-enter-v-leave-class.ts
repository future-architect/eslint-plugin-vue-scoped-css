    ...
    return {
      "Program:exit"() {
        for (const transition of getElements(
          ...
          const nodes = getAttributeValueNodes(transition, "name", context);
          if (!nodes) {
            // unknown
            return;
          }
          if (!nodes.length) {
            if (!hasEnterClass) {
              addDeprecatedClassName(Template.of("v-enter"), "enter");
              addRenamedClassName(Template.of("v-enter-from"));
            }
            if (!hasLeaveClass) {
              addDeprecatedClassName(Template.of("v-leave"), "leave");
