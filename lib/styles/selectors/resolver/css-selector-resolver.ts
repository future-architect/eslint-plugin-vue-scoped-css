...
export class CSSSelectorResolver {
  ...
  private isIgnoreNode(node: VCSSNode) {
    return (
      isVCSSAtRule(node) && node.name === "keyframes" && node.identifier === "@"
    );
  }
  ...
  protected resolveSelectorForNestContaining(
    ...
    if (parentSelectors) {
      resolved = parentSelectors.selectors.map((p) => {
        ...
        const needJoinLeft = maybeJoinLeft && isTypeSelector(parentSelector[0]);
        const needJoinRight =
          maybeJoinRight &&
    ...
    while (!selectorNodes.includes(nestingTargetNode)) {
      ...
      const before = parent.parent.nodes.slice(0, index) as VCSSSelectorNode[];
      const after = parent.parent.nodes.slice(index + 1) as VCSSSelectorNode[];
